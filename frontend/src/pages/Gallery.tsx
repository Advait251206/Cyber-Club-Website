        ...
        <div className="pt-24 pb-12 px-6 max-w-7xl mx-auto text-white">
            ...
            <div className="flex flex-wrap justify-center gap-4 mb-12">
                {filterCategories.map(category => (
                    <button
                        ...
                        className={`px-6 py-2 rounded-lg font-semibold transition-all duration-300 ${activeFilter === category ? 'bg-green-500 text-white shadow-lg shadow-green-500/30' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
                    >
                        {category}
                    </button>
                ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
                {filteredItems.map(item => (
                    <div
                        key={item.id}
                        className="group relative h-60 rounded-2xl overflow-hidden bg-black border border-green-400/50 shadow-lg hover:shadow-green-400/50 cursor-pointer transform hover:scale-105 transition-all duration-300"
                        onClick={() => openLightbox(item, 0)}
                        style={{ animation: `fadeInUp 0.5s ease-out` }}
                    >
                        <img src={item.imageUrl} alt={item.title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
